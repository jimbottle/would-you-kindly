...
func TestRender_NoEscapeSurvives(t *testing.T) {
	...
	src := &stubSource{issues: []beads.Issue{{ID: "a-1", Title: evil, Description: evil, Notes: evil, Status: "open"}}}
	m := New(src)
	model, _ := m.Update(tea.WindowSizeMsg{Width: 120, Height: 30})
	m = model.(Model)
	m = applyFetched(m, src)
	if strings.ContainsRune(m.View(), 0x1b) {
		t.Error("list View leaked a raw ESC from a hostile title")
	}
	// Detail view (description + notes rendered).
	dm, _ := m.Update(tea.KeyMsg{Type: tea.KeyEnter})
	m = dm.(Model)
	if strings.ContainsRune(m.View(), 0x1b) {
		t.Error("detail View leaked a raw ESC from a hostile description/notes")
	}
}
